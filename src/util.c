...
bool PrintErrorHandler(U_LONG type, struct SGFInfo *sgfc, va_list arglist) {
	...
	if((type & E_ACCUMULATE))			/* accumulate error messages? */
	{
		...
		{
			if(sgfc->_util_c->acc_count)	/* any errors already accumulated? */
			{
				/* different error at follow-up position? -> flush */
				if((sgfc->_util_c->acc_row != row) ||
				   (sgfc->_util_c->acc_col + sgfc->_util_c->acc_count != col) ||
				   ((sgfc->_util_c->acc_type & M_ERROR_NUM)!=(type & M_ERROR_NUM)))
				{
					PrintError(sgfc->_util_c->acc_type, sgfc,
							   sgfc->_util_c->acc_row, sgfc->_util_c->acc_col, false);
					sgfc->_util_c->acc_row = row;	/* set new */
					sgfc->_util_c->acc_col = col;
					sgfc->_util_c->acc_type = type;
				}
			}
			else								/* first error */
			{
				sgfc->_util_c->acc_type = type;	/* set data */
				sgfc->_util_c->acc_row = row;
				sgfc->_util_c->acc_col = col;
			}
			...
			while(sgfc->_util_c->acc_count + illegal_count >= ACCUMULATE_SIZE)
			{
				if(sgfc->_util_c->acc_count < ACCUMULATE_SIZE)
				{
					size_t chunk = ACCUMULATE_SIZE - sgfc->_util_c->acc_count;
					strnpcpy(&sgfc->_util_c->accumulate[sgfc->_util_c->acc_count], illegal, chunk);
					sgfc->_util_c->acc_count += chunk;
					illegal_count -= chunk;
					illegal += chunk;
				}
			...
			if(illegal_count)
			{
				strnpcpy(&sgfc->_util_c->accumulate[sgfc->_util_c->acc_count], illegal, illegal_count);
				sgfc->_util_c->acc_count += illegal_count;
			}
		...
		if(sgfc->_util_c->acc_count)	/* any errors waiting? */
	...
	if(print_c)
		malloc_size += sgfc->_util_c->acc_count + 4; /* 3 Bytes ""\n + 1 reserve */
	...
	else
	{
		...
		{
			*msg_cursor++ = '"';
			/* _util_c->accumulate string is _not_ terminated with '\0' */
			strncpy(msg_cursor, sgfc->_util_c->accumulate, sgfc->_util_c->acc_count);
			msg_cursor += sgfc->_util_c->acc_count;
			sgfc->_util_c->acc_count = 0;
			*msg_cursor++ = '"';
			*msg_cursor++ = '\n';
		}
		if(val_pos)
